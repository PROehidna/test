...
//
//Цель: разработать программу для вывода чисел в четверичной системе счисления на семисегментном индикаторе
//
...
int main(void)
{
	...
	uint16_t input[8] = {1, 1, 1, 0, 0, 0, 0, 0} ;																			//Входное занчение числа (в двоичной системе)
	...
	unsigned reg[9] = {0x0000023F, 0x00000206, 0x0000025B, 0x0000024F, 0x266, 0x26D, 0x27D, 0x207, 0x27F};									//Регистры семисегментного индикатора, цифры 0,1,2,3 соответственно
	//Реализация вывода на семисегментный индикатор
	while(1){
		n=0;
		input[3]=((GPIOB->IDR)&0x8000)>>15;																								//Считывание значения переключателя SW1
		input[5]=((GPIOB->IDR)&0x4000)>>14;																								//Считывание значения переключателя SW2
		...
		for (uint16_t i = 0 ; i<9; i++){       
			if(n<9) {																																				//условие срабатывает, если десятичное число меньше разрядности
				output[i] = n;																																//в выходной массив записывается значение числа
				break;																																				//цикл прерывается
			}
			output[i] = n%9;																																//в выходной массив записывается остаток от деления на 4 
			n = n/9;																																				//в число n записывается целая часть от деления
		} 
