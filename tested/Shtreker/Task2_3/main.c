...
**Разработчик: Штрекер Владислав Александрович - 1193б
...
**				4. Перевод кода клавиши в систему исчесления с основанием 5
...
int main(void)
{
	...
	uint8_t text[10] = {0x83,0x76,0x61,0x2D,0x34,0x3E,0x20,0x20, 0x20, 0x20};						//Массив содержащий сообщение "sva-4>    "
	while(1){
		//Сброс массива text
		...
		//Отпрвака данных в USART (для очистки консоли)
			for (uint8_t i=0; i<10; i++){																										//Повтор действия 10 раз (количество выведенных элементов в консоли
		...
		for (uint16_t i = 9 ; i>5; i--){
			if(d<4) {																																				//условие срабатывает, если десятичное число меньше разрядности
				text[i] = d+48;																																//в выходной массив записывается значение числа
				break;																																				//цикл прерывается
			}
			text[i] = d%4+48;																																//в выходной массив записывается остаток от деления на 4
			d = d/4;																																				//в число записывается целая часть от деления
		}
...
void InitUSART1(){
	...
	USART1->BRR= 35;																																		/*Настройка делителя частоты, тактирующего USART и задающего скорость приема и передачи данных на уровне 9600 бит/с:
																																												Частота тактирующего генератора = 8 МГц
																																												Скорость обмена по USART - 9600 бит/с; коэффициент деления - 8000000 / 230400 - 34,7(2); Округленное значение - 35*/
	USART1->CR1 = 0xc;																																	/*Разрешить работу приемника и передатчика USART. Остальные биты этого регистра сброшены, что обеспечивает:
																																												...
																																												контроль четности - отключен (не работает); 
																																												прерывания по любым флагам USART - запрещены;
																																												состояние USART - отключен*/
	USART1->CR2 = 0x3000;																																		//Количество стоповых бит - 1.5
