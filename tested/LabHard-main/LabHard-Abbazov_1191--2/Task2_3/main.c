...
int main(void)
{
	...
	while(1){
		...
		for (uint16_t i = 9 ; i>5; i--){       
			...
			d = d/5;																																				//в число записывается целая часть от деления
		} 
...
void InitUSART1(){
	...
	USART1->BRR= 833;																																		/*Настройка делителя частоты, тактирующего USART и задающего скорость приема и передачи данных на уровне 9600 бит/с: 
																																												Частота тактирующего генератора = 8 МГц 
																																												Скорость обмена по USART - 9600 бит/с; коэффициент деления - 8000000 / 9600 - 833,333(3); Округленное значение - 833*/
