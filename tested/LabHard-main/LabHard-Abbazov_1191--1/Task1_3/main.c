...
int main(void)
{
	...
	unsigned reg[4] = {0x0000023F, 0x00000206, 0x0000025B, 0x0000024F};									//Регистры семисегментного индикатора, цифры 0,1,2,3 соответственно
	...
	while(1){
		...
		for (uint16_t i = 0 ; i<4; i++){       
			if(n<4) {																																				//условие срабатывает, если десятичное число меньше разрядности
				output[i] = n;																																//в выходной массив записывается значение числа
				break;																																				//цикл прерывается
			}
			output[i] = n%4;																																//в выходной массив записывается остаток от деления на 4 
			n = n/4;																																				//в число n записывается целая часть от деления
		} 
